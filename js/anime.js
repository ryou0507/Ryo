...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const scrollAnimationTargets = document.querySelectorAll(
    '.centered-image img'
  ); // このセレクタはアニメーション対象の画像に合わせて調整してください
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const elements = document.querySelectorAll(
    '.plan-container1 h3, .plan-container1 p, .plan-container2 h3, .plan-container2 p, .plan-container3 h3, .plan-container3 p'
  );
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  audioElements.forEach((audioElement) => {
    console.log('Setting up audio:', audioElement.getAttribute('data-src'));
    let player;

    audioElement.addEventListener('play', function () {
      console.log('Play event triggered');
      if (!player) {
        console.log('Initializing Howl');
        player = new Howl({
          src: [audioElement.getAttribute('data-src')],
          autoplay: true,
          loop: audioElement.hasAttribute('loop'),
          volume: parseFloat(audioElement.getAttribute('volume')) || 0.5,
          onload: function () {
        ...
        console.log('Playing existing player');
        player.play();
      }
    });
